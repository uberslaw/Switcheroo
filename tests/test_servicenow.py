...
def test_approve_and_reject_update_status_and_dry_run_file(seeded_db):
    ...
    req2 = create_request(seeded_db, cs, port2, REQUEST_VLAN, vlan_id=40)
    seeded_db.flush()
...
def test_poll_is_noop_http_in_dry_run(seeded_db):
    ...
    assert all(r.get("number") == DRY_RUN_TICKET for r in rows)
...
def test_live_create_posts_table_api_when_mocked(seeded_db, monkeypatch):
    ...
    req = create_request(seeded_db, cs, port, REQUEST_VLAN, vlan_id=50)
    seeded_db.commit()
    assert req.servicenow_ticket == "INC0012345"
    assert req.servicenow_sys_id == "abc123sys"
    assert fake.calls
